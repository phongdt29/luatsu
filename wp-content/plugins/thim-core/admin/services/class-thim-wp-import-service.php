...
//		die;
	...
	public function analyze_content( $xml ) {
		...
		if ( $data['posts'] ) {
			...
			if ( ! empty( $post_types['lp_lesson'] ) ) {
				$pp['lp_lesson'] = $post_types['lp_lesson'];
				unset( $post_types['lp_lesson'] );
			}

			if ( ! empty( $post_types['lp_question'] ) ) {
				$pp['lp_question'] = $post_types['lp_question'];
				unset( $post_types['lp_question'] );
			}

			if ( ! empty( $post_types['lp_quiz'] ) ) {
				$pp['lp_quiz'] = $post_types['lp_quiz'];
				unset( $post_types['lp_quiz'] );
			}

			if ( ! empty( $post_types['lp_course'] ) ) {
				$pp['lp_course'] = $post_types['lp_course'];
				unset( $post_types['lp_course'] );
			}
	...
	private function _clean_posts() {
		...
		if ( $post_ids ) {
			...
			// learnpress_sections
			$lp_sections        = $wpdb->prefix . 'learnpress_sections';
			$lp_section_item        = $wpdb->prefix . 'learnpress_section_items';
			$quiz_item        = $wpdb->prefix . 'learnpress_quiz_questions';
			$question_answers       = $wpdb->prefix . 'learnpress_question_answers';
 			$wpdb->query(
				$wpdb->prepare(
					"DELETE FROM {$lp_sections} WHERE section_course_id IN (" . join( ",", $in ) . ")",
					$post_ids
				)
			);
			$wpdb->query(
				$wpdb->prepare(
					"DELETE FROM {$lp_section_item} WHERE item_id IN (" . join( ",", $in ) . ")",
					$post_ids
				)
			);
			$wpdb->query(
				$wpdb->prepare(
					"DELETE FROM {$quiz_item} WHERE quiz_id IN (" . join( ",", $in ) . ")",
					$post_ids
				)
			);
			$wpdb->query(
				$wpdb->prepare(
					"DELETE FROM {$question_answers} WHERE question_id IN (" . join( ",", $in ) . ")",
					$post_ids
				)
			);
		}
